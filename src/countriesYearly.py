...
for date in dateListCopy:
    if date == '22-12-31':
        yearlyDateList.append(date)
    elif dateListCopy[dateDayCount][-5:] == '01-01':
        yearlyDateList.append(date)
    dateDayCount += 1
...
for countryCode in countryCodeList:
    countryTagCountList = []
    countryTagCountYear = 0
    for date in model:
        dateData = model[date]
        if countryCode in dateData:
            countryTagCountYear += dateData[countryCode]
        else:
            countryTagCountYear += 0
        if date in x:
            countryTagCountList.append(countryTagCountYear)
            countryTagCountYear = 0
        else:
            continue
...
plt.savefig(args.key + 'Yearly.png')